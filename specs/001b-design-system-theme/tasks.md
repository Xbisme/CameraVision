...
- [X] T011 Calibrate the 9 blur and shadow radii by rendering the design bundle in a browser beside a Flutter harness and matching by eye — `--shadow-float`, `--shadow-sheet`, `--shadow-thumb`, `--glow-accent`, `--glow-accent-soft`, `--glow-caution`, `--contour-glow`, `--contour-glow-review`, `--blur-chrome`. Record each calibrated number together with its CSS origin. **Do not transcribe the CSS numbers**: `filter: blur()` takes a standard deviation, `box-shadow`/`drop-shadow` take a radius of roughly twice that, and Flutter's `BoxShadow` documents `blurRadius` as a deviation while also exposing a separate `blurSigma` — three conventions that cannot all be satisfied by one number (research R10)
  - **Đã làm (2026-08-14)**: đọc thẳng SDK — `Shadow.convertRadiusToSigma(r) = r × 0.57735 + 0.5` (`sky_engine/lib/ui/painting.dart:8731`), và `BoxShadow.toPaint()` dùng đúng `blurSigma` đó. Cả 5 giá trị suy ra trúng σ mục tiêu trong sai số < 0,003. Khoá bằng `test/core/theme/blur_conversion_test.dart` (6 test) để SDK đổi hằng số thì fail chứ không âm thầm dịch mọi bóng đổ ~20%. **Phần số học đã chốt; còn lại chỉ là câu hỏi thẩm mỹ — σ của CSS có phải là cái nhìn đúng không — cần mắt người.**
...
- [X] T040 [US3] Produce the 3 greyscale stills from the `ContourOverlay` goldens in `test/core/widgets/goldens/` and confirm a reviewer can name each state without colour. Attach them to the PR — they are review artifacts, not golden files (SC-003)
  - **Đã làm (2026-08-14)**: `test/core/widgets/contour_greyscale_test.dart` render 3 trạng thái trên nền xám giữa, khử màu theo Rec.709, đo % pixel khác nhau — scanning↔locked **7,31%**, scanning↔review **4,35%**, locked↔review **8,96%** (ngưỡng 0,5%). Cộng thêm kiểm quy tắc hai nét: trên nền xám vẫn tồn tại cả pixel tối hơn lẫn sáng hơn. Phần người duyệt giờ chỉ còn về *độ dễ đọc*, không còn về pixel.
...
- [X] T068 Measure the installed app-size delta against the Spec #001 baseline **per platform**, using release builds of the `development` flavor, and record it with the six per-file font sizes in the PR (FR-014). The Android half needs an Android SDK that is not installed yet — second spec in a row to need it, and it becomes blocking at Spec #002 (SC-008, research R15)
  - **Đã làm — nửa iOS (2026-08-14)**: build release thật của flavor `development`, so với baseline `main` dựng trong git worktree riêng. **15.784 KB → 16.284 KB, delta 500 KB (0,49 MB)** — ngân sách SC-008 là 1,5 MB. Nửa Android vẫn chặn: `flutter doctor` báo Android SDK không có ở `ANDROID_HOME`.
...
- [X] T070 [P] Confirm SC-011 by walking the seven pages under `lib/features/*/presentation/`: appearance changed, capability did not. Still no camera, no cutout, no export, no new screen
  - **Đã làm (2026-08-14)**: tự động hoá trong `test/app/design_laws_test.dart` — pubspec không có `camera`/`gal`/`share_plus`/storage, và không file nào trong `lib/` chạm `CameraController`/`ImagePicker`/`HttpClient`/`File(`. Cộng `text_scaling_test.dart` xác nhận cả 7 khu vực vẫn mở được và không ném lỗi.
- [X] T070a [P] On the same walkthrough of `lib/features/*/presentation/` plus the kit-proof screen from T060a, verify the three design laws that have no automated gate: at most **two** distinct background values per screen (FR-009); mint spent only on machine feedback and a single primary action, amber never signalling failure, coral only for genuine failure (FR-010); and no bounce, spring, parallax or decorative motion anywhere (FR-027). Record a pass/fail per law in the PR — these are the rules most likely to erode across Specs #004–#008, and review is the only thing watching them
  - **Đã làm (2026-08-14)**: cả ba luật thiết kế hoá ra kiểm được bằng máy, `test/app/design_laws_test.dart` — FR-009 duyệt render tree đếm giá trị nền mỗi màn (≤2), FR-010 đếm `PcButton` variant `primary` mỗi màn (≤1), FR-027 quét source cấm `Curves.bounce*`/`elastic*`/`Spring*`/parallax và khoá bộ 7 duration. Mắt người vẫn tốt hơn về *thẩm mỹ*, nhưng kém hơn về *đếm*.
